// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Linq;
using Azure.Core;

namespace Microsoft.Azure.IoTCentral.Preview.Models
{
    /// <summary> Configuration specifying options for a map property tile. </summary>
    public partial class MapPropertyConfiguration : TileConfiguration
    {
        /// <summary> Initializes a new instance of MapPropertyConfiguration. </summary>
        /// <param name="group"> The ID of the device group to display. </param>
        /// <param name="devices"> The list of associated devices to display. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="group"/> or <paramref name="devices"/> is null. </exception>
        public MapPropertyConfiguration(string group, IEnumerable<string> devices)
        {
            Argument.AssertNotNull(group, nameof(group));
            Argument.AssertNotNull(devices, nameof(devices));

            Group = group;
            Devices = devices.ToList();
            Capabilities = new ChangeTrackingList<TileCapability>();
            Type = "mapProperty";
        }

        /// <summary> Initializes a new instance of MapPropertyConfiguration. </summary>
        /// <param name="type"> The type of widget the tile renders. </param>
        /// <param name="zoomLevel"> The zoom level of the map. </param>
        /// <param name="group"> The ID of the device group to display. </param>
        /// <param name="devices"> The list of associated devices to display. </param>
        /// <param name="capabilities"></param>
        internal MapPropertyConfiguration(string type, float? zoomLevel, string group, IList<string> devices, IList<TileCapability> capabilities) : base(type)
        {
            ZoomLevel = zoomLevel;
            Group = group;
            Devices = devices;
            Capabilities = capabilities;
            Type = type ?? "mapProperty";
        }

        /// <summary> The zoom level of the map. </summary>
        public float? ZoomLevel { get; set; }
        /// <summary> The ID of the device group to display. </summary>
        public string Group { get; set; }
        /// <summary> The list of associated devices to display. </summary>
        public IList<string> Devices { get; }
        /// <summary> Gets the capabilities. </summary>
        public IList<TileCapability> Capabilities { get; }
    }
}
