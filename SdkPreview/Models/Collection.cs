// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Linq;
using Azure.Core;

namespace Microsoft.Azure.IoTCentral.Preview.Models
{
    /// <summary> The collection of entities. </summary>
    public partial class Collection
    {
        /// <summary> Initializes a new instance of Collection. </summary>
        /// <param name="value"> The collection of entities. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="value"/> is null. </exception>
        internal Collection(IEnumerable<object> value)
        {
            Argument.AssertNotNull(value, nameof(value));

            Value = value.ToList();
        }

        /// <summary> Initializes a new instance of Collection. </summary>
        /// <param name="value"> The collection of entities. </param>
        /// <param name="nextLink"> URL to get the next page of entities. </param>
        internal Collection(IReadOnlyList<object> value, string nextLink)
        {
            Value = value;
            NextLink = nextLink;
        }

        /// <summary> The collection of entities. </summary>
        public IReadOnlyList<object> Value { get; }
        /// <summary> URL to get the next page of entities. </summary>
        public string NextLink { get; }
    }
}
